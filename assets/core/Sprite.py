...
class Sprite(ISprite):
    ...
    def validatePosition(self):
        self.position.x = min(float(GAME_WIDTH) - self.halfImageSize.x * 0.5, max(self.position.x, self.halfImageSize.x * 0.5))
        self.position.y = min(float(GAME_HEIGHT) - self.halfImageSize.y * 0.5, max(self.position.y, self.halfImageSize.y * 0.5))
